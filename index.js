...
// =========================================================================
// 🚀 INYECTOR MASIVO AUTO-CONSTRUIBLE PARA AGROPETS (CREA TABLA + INYECTA)
// =========================================================================
app.get('/api/admin/cargar-agropets', (req, res) => {
    
    // 1. Comando blindado para crear la tabla automáticamente si no existe en Clever Cloud
    const sqlCrearTabla = `
    ...
    db.query(sqlCrearTabla, (errTabla) => {
        if (errTabla) {
            console.error("❌ Error creando la tabla productos en la nube:", errTabla.message);
            return res.status(500).send("Error al crear la estructura de la tabla: " + errTabla.message);
        }

        // 2. Una vez asegurada la tabla, procedemos con la lista de tus 75 productos
        const listaAgropets = [
        ...
        db.query(queryInsertar, [listaAgropets], (errInsert, result) => {
            if (errInsert) {
                console.error("❌ Error inyectando catálogo total:", errInsert.message);
