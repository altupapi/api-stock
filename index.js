...
// =========================================================================
// 🚀 INYECTOR MASIVO BINDADO (BORRA TABLA VIEJA + CREA NUEVA + INYECTA)
// =========================================================================
app.get('/api/admin/cargar-agropets', (req, res) => {
    
    // 1. Forzamos el borrado de la tabla vieja con columnas incorrectas si existía
    const sqlBorrarTabla = "DROP TABLE IF EXISTS productos;";

    db.query(sqlBorrarTabla, (errDrop) => {
        if (errDrop) {
            console.error("❌ Error al eliminar tabla vieja:", errDrop.message);
            return res.status(500).send("Error al limpiar base de datos: " + errDrop.message);
        }

        // 2. Creamos la tabla 100% limpia con la estructura oficial ('cantidad' y 'precio')
        const sqlCrearTabla = `
        ...
        db.query(sqlCrearTabla, (errTabla) => {
            if (errTabla) {
                console.error("❌ Error creando la tabla productos limpia:", errTabla.message);
                return res.status(500).send("Error al crear la estructura limpia: " + errTabla.message);
            }

            // 3. Lista completa de tus 75 productos listos para impactar
            const listaAgropets = [
                ...
                ["Cerdos Eco Pig 3 Micro Pellet 25kg (B025)", 4, 109.00, "Agrícola - Cerdos"],
            ...
            const queryInsertar = 'INSERT INTO productos (nombre, cantidad, precio, categoria) VALUES ?';
            
            db.query(queryInsertar, [listaAgropets], (errInsert, result) => {
                if (errInsert) {
                    console.error("❌ Error inyectando catálogo:", errInsert.message);
                    return res.status(500).send("Error en la inyección masiva: " + errInsert.message);
                }
                res.send(`<h1>🎉 ¡Base de datos reestructurada y limpia con éxito!</h1><p>Se registraron exitosamente los <b>${result.affectedRows}</b> productos en Clever Cloud.</p>`);
            });
        });
    });
});
